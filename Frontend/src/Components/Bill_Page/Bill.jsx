...
const Bill = ({ enrollmentId, onClose }) => {
  ...
  useEffect(() => {
    ...
    // ✅ Fixed: backticks for template string
    axiosInstance
  ...
  const executePrint = () => {
    ...
    // ✅ Fixed: document.write must be a string using backticks
    ...
    setTimeout(() => {
      ...
      printWindow.print();
      printWindow.close();
      if (onClose) onClose(); // close parent modal automatically after print
    }, 300);
  };

  // ================= AUTO PRINT WHEN ENROLLMENT LOADS =================
  useEffect(() => {
    if (enrollment) {
      executePrint();
    }
